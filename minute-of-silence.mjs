...
constructor(clock,onState=()=>{}){this.clock=clock;this.onState=onState;this.context=null;this.buffer=null;this.source=null;this.releaseLock=null;this.plan=null;this.busy=false;this.claiming=false;this.state='waiting';this.loadError=false;this.stopped=false;this.tabId=globalThis.crypto?.randomUUID?.()||String(Math.random());this.leaseKey='kvitneve.audio.lease';this.fallback=false;this.leasePending=false;
...
if(navigator.locks?.request){navigator.locks.request('kvitneve-minute-of-silence',{ifAvailable:true},async lock=>{if(!lock)return;await new Promise(resolve=>{this.releaseLock=resolve;this.start();if(!this.source){this.releaseLock=null;resolve();}});}).catch(()=>{this.state='blocked';}).finally(()=>{this.claiming=false;this.publish();});}
