...
const files=['index.html','class.html','print.html','print-preview.css','print-preview.mjs','print-posters.mjs','print-subject-icons.mjs','Розклад дзвінків.html','site.css','enhancements.css','highlights.css','responsive.css','ukraine.css','print.css','print-layout.mjs','theme-init.js','effects.mjs','schedule-view.mjs','subject-icons.mjs','app.mjs','lesson-content.mjs','table-navigation.mjs','kvitneve.css','schedule-config.js','time-core.mjs','kyiv-clock.mjs','minute-of-silence.mjs','school-bell.mjs','favicon.svg','school-background-2026.png','THIRD_PARTY_NOTICES.md','Хвилина мовчання.mp3','Звук шкільного дзвінка.mp3'];
...
files.push(...fs.readdirSync('assets/print').filter(x=>/\.(png|webp|json)$/.test(x)).map(x=>'assets/print/'+x));
files.push(...fs.readdirSync('assets/print-subjects').filter(x=>x.endsWith('.svg')).map(x=>'assets/print-subjects/'+x));
execFileSync(process.execPath,['scripts/validate-print-posters.mjs'],{stdio:'inherit'});
